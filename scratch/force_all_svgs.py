...
def replace_icon_wrapper(content, title, new_svg):
    # Regex to find <div class="icon-wrapper">...</div> just before <div class="service-content">...<h3>TITLE</h3>
    pattern = r'(<div class="icon-wrapper">)[\s\S]*?(</div>\s*<div class="service-content">\s*<h3>' + re.escape(title) + r'</h3>)'
    replacement = r'\1\n                          ' + new_svg + r'\n                      \2'
...
for country in countries:
    ...
    if os.path.exists(html_path):
        ...
        content = replace_icon_wrapper(content, 'BUSINESS VISA', new_svg_business)
        content = replace_icon_wrapper(content, 'WORK VISA', new_svg_work)
        content = replace_icon_wrapper(content, 'TOURIST VISA', new_svg_tourist)
        content = replace_icon_wrapper(content, 'STUDENT VISA', new_svg_student)

        with open(html_path, 'w', encoding='utf-8') as f:
            f.write(content)
            
print("Icons rigidly forced to correct SVGs across ALL countries.")
