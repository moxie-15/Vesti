...
try:
    ...
    # Extract the hero section from NZ (find <!-- Hero Section --> and the very next </section>)
    match = re.search(r'<!-- Hero Section.*?<section.*?</section>', nz_content, re.DOTALL | re.IGNORECASE)
    if not match:
    ...
    for code, data in countries.items():
        ...
        # Remove old hero from destination file (it might have style attrs)
        content = re.sub(r'<!-- Hero Section.*?<section.*?</section>', new_hero, content, count=1, flags=re.DOTALL | re.IGNORECASE)
