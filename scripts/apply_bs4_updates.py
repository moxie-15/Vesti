...
<style id="custom-dropdown-css">
    ...
    .nav-menu .dropdown-menu li { padding: 0; margin: 0; }
...
for filepath in html_files:
    try:
        ...
        # Add CSS
        if not soup.find(id="custom-dropdown-css"):
            style_tag = BeautifulSoup(css_addition, 'html.parser')
            if soup.head:
        ...
        # Add script for modal if not exists
        if not soup.find(id="agent-modal-script"):
...
print("DOM precise update complete.")
